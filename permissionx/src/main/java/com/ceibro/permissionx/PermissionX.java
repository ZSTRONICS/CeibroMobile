...
public class PermissionX {
    ...
    public static String localizedPermissionNames(Context context, List<String> deniedList) {
        StringBuilder builder = new StringBuilder();
        try {
            for (String permission : deniedList) {
                CharSequence name = context.getPackageManager().getPermissionGroupInfo(permission, 0).loadLabel(context.getPackageManager());
