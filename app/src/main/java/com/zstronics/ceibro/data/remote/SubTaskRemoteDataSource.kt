...
class SubTaskRemoteDataSource @Inject constructor(private val service: TaskRetroService) :
    ...
    override suspend fun updateMemberInSubTask(
        ...
        executeSafely(
            call =
            {
                service.updateMemberInSubTask(subtaskId, addMemberSubTask)
            }
        )
    ...
    override suspend fun getAllCommentsOfSubtask(subTaskId: String): ApiResponse<AllCommentsResponse> =
        executeSafely(
            call =
            {
                service.getAllCommentsOfSubtask(subTaskId)
            }
        )
}