...
class GroupConnectionAdapterSectionRecycler constructor(
    val context: Context,
    val sectionList: MutableList<GroupConnectionsSectionHeader>
) :
    SectionRecyclerViewAdapter<GroupConnectionsSectionHeader, TaskMemberDetail, GroupConnectionAdapterSectionRecycler.ConnectionsSectionViewHolder, GroupConnectionAdapterSectionRecycler.ConnectionsChildViewHolder>(
        context,
        sectionList
    ) {
    ...
    var itemClickListener: ((view: View, position: Int, data: TaskMemberDetail) -> Unit)? =
        null

    var fullItemClickListener: ((view: View, position: Int, data: TaskMemberDetail) -> Unit)? =
        null
    var dataList: MutableList<TaskMemberDetail> = mutableListOf()
    var disableContscts: MutableList<TaskMemberDetail> = mutableListOf()
    ...
    override fun onBindChildViewHolder(
        ...
        p1: Int,
        p2: Int,
        connectionItem: TaskMemberDetail?
    ) {
    ...
    override fun onBindSectionViewHolder(
        ...
        p1: Int,
        GroupConnectionsSectionHeader: GroupConnectionsSectionHeader?
    ) {
        connectionsSectionViewHolder?.bind(GroupConnectionsSectionHeader)
    }
    ...
    fun disableContacts(oldSelectedContacts: MutableList<TaskMemberDetail>) {
    ...
    inner class ConnectionsSectionViewHolder constructor(private val binding: LayoutItemHeaderBinding) :
        ...
        fun bind(item: GroupConnectionsSectionHeader?) {
        ...
        fun bind(item: TaskMemberDetail?) {
            ...
            item?.let {
                with(binding) {
                    ...
                    contactName.text = "${item.firstName} ${item.surName}"


                        phoneNumber.text = ""
                        phoneNumber.visibility = View.GONE

                        if (item.companyName?.trim().isNullOrEmpty()) {
                            companyName.visibility = View.GONE
                            dot.visibility = View.GONE
                        } else {
                            companyName.visibility = View.VISIBLE
                            companyName.text = item.companyName?.trim()
                        }

                        if (item.jobTitle?.trim().isNullOrEmpty()) {
                            dot.visibility = View.GONE
                            ...
                            jobTitle.text = item.jobTitle?.trim()
                        }
