...
@HiltViewModel
class TasksHiddenParentTabV3VM @Inject constructor(
    override val viewState: TasksHiddenParentTabV3State,
    ...
    fun loadAllTasks(callBack: () -> Unit) {
        launch {
                ...
                rootAllHiddenTasksDB.filter {
                    it.taskRootState.equals(TaskRootStateTags.Hidden.tagValue, true) &&
                            (it.hiddenState.equals(
                                ...
                                true
                            ) || it.hiddenState.equals(TaskStatus.ONGOING.name, true)))
                }.sortedByDescending { it.updatedAt }.toMutableList()

            val rootHiddenClosedTasksDB =
                rootAllHiddenTasksDB.filter {
                    it.taskRootState.equals(TaskRootStateTags.Hidden.tagValue, true) &&
                            (it.hiddenState.equals(
            ...
            filteredOngoingTasks = rootHiddenOngoingTasksDB
            _ongoingAllTasks.postValue(rootHiddenOngoingTasksDB)
            originalOngoingAllTasks = rootHiddenOngoingTasksDB

            filteredClosedTasks = rootHiddenClosedTasksDB
            _closedAllTasks.postValue(rootHiddenClosedTasksDB)
            originalClosedAllTasks = rootHiddenClosedTasksDB
