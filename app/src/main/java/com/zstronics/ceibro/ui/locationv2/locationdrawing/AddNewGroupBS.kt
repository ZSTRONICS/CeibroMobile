...
import com.zstronics.ceibro.databinding.FragmentAddNewGroupBsheetBinding
...
@AndroidEntryPoint
class AddNewGroupBS(
    val model: LocationDrawingV2VM,
    val callback: (group: CeibroGroupsV2) -> Unit
) :
    BottomSheetDialogFragment() {
    lateinit var binding: FragmentAddNewGroupBsheetBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(
            inflater,
            R.layout.fragment_add_new_group_bsheet,
            container,
            false
        )
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        model.originalGroups.observe(viewLifecycleOwner) {
                ...
                if (binding.addGroupBtn.text.toString().equals("Rename", true)) {
                    groupDataToUpdate?.let {
                        if (!it.groupName.equals(text, true)) {
                            ...
                            if (model.originalGroups.value?.any {
                                    it.groupName.equals(
                                        text,
                                        true
                                    )
                    ...
                    if (model.originalGroups.value?.any { it.groupName.equals(text, true) } == true) {
                        shortToastNow("Group name already exists")
                    } else {
